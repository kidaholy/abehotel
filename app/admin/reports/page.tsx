    ...
    const exportFinancialReport = () => {
        const data = [
            ...
            ...cashierRevenue.map(c => ({
                ...
                Description: `Generated by ${c.name} (${Object.entries(c.breakdowns).map(([k,v]) => `${k}: ${v} ETB`).join(", ")})`
            })),
                                                            ...
                                                            {cashierRevenue.map((c, idx) => (
                                                                <tr key={idx} className="hover:bg-white/5 transition-colors border-l-2 border-[#d4af37]/30">
                                                                    ...
                                                                    <td className="p-4 text-gray-500 text-[10px] italic">
                                                                        ...
                                                                        {Object.keys(c.breakdowns).length > 0 && (
                                                                             <div className="flex flex-wrap gap-2 mt-1.5">
                                                                                 {Object.entries(c.breakdowns).sort((a,b) => b[1]-a[1]).map(([label, val]) => (
                                                                                     <span key={label} className="bg-white/5 border border-white/10 px-1.5 py-0.5 rounded text-[8px] tracking-widest uppercase not-italic text-gray-400">
                                                                                         {label}: <span className="text-[#f3cf7a]">{val.toLocaleString()} Br</span>
                                                                                     </span>
                                                                                 ))}
                                                                             </div>
                                                                        )}
